...
<body>
    ...
    <div class="m-lg-3">
        <a 
        href="" 
        onclick="seleccionaPais(<?php echo 1 . ',' . 1 ?>);" 
        class="btn btn-success regresar_Btn" 
        data-bs-toggle="modal" 
        data-bs-target="#modalPaises">
        Agregar
      </a>
        <a href="<?php echo base_url('eliminados_paises'); ?>" class="btn btn-danger">Eliminados</a>
          ...
          <tbody>
            <?php foreach( $pais as $row ) { ?>
              <tr>
                ...
                <input 
                  href="#" 
                  onclick="seleccionaPais(<?php echo $row['id'] . ',' . 2 ?>);" 
                  data-bs-toggle="modal" 
                  data-bs-target="#modalPaises" 
                  type="image" 
                ...
                <input 
                  href="#" 
                  data-href="<?php echo base_url('paises/cambiarEstadoPais') . '/' .$row['id']. '/' .'E'; ?>" 
                  data-bs-toggle="modal" 
    ...
    <!-- Modal -->
    <form method="POST" action="<?php echo base_url('paises/insertarPais') ?>" autocomplete="off">
      <div class="modal fade" id="modalPaises" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            <div class="modal-header bg-success">
              <h1 class="modal-title fs-5" id="tituloModal">Agregar un nuevo país</h1>
              <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
              <input id="tp" name="tp" >
              <input id="id" name="id" hidden>
              <input type="text" placeholder='Codigo del país' class='input_valor' id="codigo" name="codigo" required maxLength="4">
              <input type="text" placeholder='Nombre del país' class='input_valor' id="nombre" name="nombre" required>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
              <button type="submit" type="button" class="btn btn-success" id="btn_Guardar">Grabar</button>
            </div>
          </div>
        </div>
      </div>
...
<script>
  ...
  function seleccionaPais(id, tp) {
    if (tp == 2) {
      dataURL = "<?php echo base_url('paises/buscarUnPais'); ?>" + "/" + id;
      $.ajax({
        type: "POST",
        url: dataURL,
        dataType: "json",
        success: function(rs) {        
          ...
          document.getElementById('tituloModal').innerText = "Actualizar País";
        }
      })
    }else{
      $("#tp").val(1);
